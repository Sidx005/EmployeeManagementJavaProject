package student.management.system;
import java.awt.Image;
import javax.swing.ImageIcon;
import javax.swing.JFrame;
import javax.swing.JLabel;

public class Splash extends  JFrame {

    Splash() {
      // Adding image to the JFrame
       ImageIcon i1=new ImageIcon(ClassLoader.getSystemResource("icons/front.gif"));  
       Image i2=i1.getImage().getScaledInstance(1170, 650, Image.SCALE_DEFAULT);// scaling the image     
       ImageIcon i3=new  ImageIcon(i2); // converting back to ImageIcon
       JLabel image=new  JLabel(i3); // adding image to JLabel
      //  JLabel is used to display image or text
       image.setBounds(0,0,1170,650);// setting position and size of image

       add(image);

      //  JFrame properties
        setSize(1170,650);
        setLocation(200,50);
        setLayout(null);
        setVisible(true);

        try {
          Thread.sleep(5000); 
          setVisible(false); 
          new Login();

        } catch (Exception e) {
          e.printStackTrace();
        }

    }


public static void main(String[] args) {
  new Splash();  
}
}
